...
class TestGetBooks:
        ...
        def test_get_all_books_filter_by_type_fiction(self):
                ...
                assert response[i]["type"] == "fiction", f"Error: Book {response[i]['name']} has an invalid type"
		def test_get_all_books_filter_by_type_fiction(self):
			    response = get_all_books(1, 'The Vanishing Half')
                assert response = 201, f"Error: status code is not valid. Expected: 201, actual: {response} "
                assert response[0]]['title'] == 'The Great Gatsby', "Unexpected book title"
                assert response [1]['title'] == 'To Kill a Mockingbird', "Unexpected book title"
                assert response [2] ['tile'] == 'Viscount Who Loved Me', 'Unexpected book title'
